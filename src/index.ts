  ...
  try {
    ...
    $(selectors.sessionList.container).each((_index, element) => {
      ...
      const linkElement = titleElement
        .parent()
        .filter(selectors.sessionList.linkElement);
      ...
      if (ltContainer.length > 0) {
        ...
        ltContainer
          .find(selectors.sessionList.ltItems)
          .each((_i, ltElement) => {
            const ltTitle = $(ltElement)
              .find(selectors.sessionList.title)
              .text()
              .trim();
            const ltSpeaker = $(ltElement)
              .find(selectors.sessionList.speaker)
              .text()
              .trim();
            let ltDetailPageUrl: string | undefined;

            const ltLinkElement = $(ltElement).find(
              selectors.sessionList.linkElement
            );
...
async function fetchSessionDetail(session: SessionInfo): Promise<SessionInfo> {
  ...
  try {
    ...
    if (speakerIcon.length > 0) {
      ...
      if (
        session.speakerIconUrl &&
        !session.speakerIconUrl.startsWith("http")
      ) {
    ...
    // 登壇者のプロフィール
    const speakerProfile = speakerSection
      .find(selectors.sessionDetail.speakerProfile)
      .first();
    ...
    console.error(
      `詳細情報の取得中にエラーが発生しました: ${session.title}`,
      error
    );
    return session;
  }
